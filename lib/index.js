(function() {
   ...
   * Use `registerPlugin` and `unregisterPlugin`
   * for registering shared plugins.
   */
  var globalPlugins = {};
  keylime.registerPlugin = registerPlugin;
  keylime.unregisterPlugin = unregisterPlugin;
  keylime.plugins = plugins;
  ...
  function keylime(name) {
    ...
    /**
     * This internal array will hold any functions that need to
     * be executed when an attribute value is set during creation
     */
    constructor._attrInitHandlers = [];
    ...
    constructor.attrInit = attrInit.bind(constructor);
  ...
  function recursiveCallHandlers() {
    ...
    /**
     * Call each attribute handler with every attribute
     * in the instance that's been created.
     */
    if (constructor._attrInitHandlers.length > 0) {
      _.forIn(constructor._blueprint, function(attr, name) {
        _.each(constructor._attrInitHandlers, function(handler) {
          handler.call(this, name, this[name], attr);
        }, this);
      }, this);
    }
  ...
  function recursiveApplyBlueprint(constructor, values) {
     ...
     */
    _.forIn(constructor._blueprint, function(attr, key) {
      var value = _.has(values, key) ? values[key] : attr.value;
   ...
   */
  function use() {
    var plugin = _.first(arguments),
        args = _.rest(arguments),
        pluginFunc;

    if (!_.isFunction(plugin) && !_.isString(plugin)) {
      throw new Error('You must supply a function or a \'name\' to call \'use\' on '+this.name+'.');
    }
    ...
    if (_.isString(plugin)) {
      if (!_.has(globalPlugins, plugin)) {
        throw new Error('No valid plugin found for with the name \''+plugin+'\'! Check that keylime.plugins(\''+plugin+'\') exists.');
      }
      pluginFunc = globalPlugins[plugin];
    }

    pluginFunc.apply(pluginFunc, _.flatten([this, args]));
  ...
  /**
   * The init method accumulates functions in an handlers array.
   * These handlers will be invoked for every attribute in the
   * model when an instance is created.
   */
  function attrInit(handler) {
    if (!_.isFunction(handler)) {
      throw new Error('You must supply a function to add an attribute initialization handler on '+this.name+'.');
    }

    this._attrInitHandlers.push(handler);

    return this;
  }

  /**
   * Registers a plugin function globally. Any plugins registered
   * globally can be referenced by name when calling `use` on
   * a keylime constructor.
   */
  ...
  function plugins(name) {
    var globals = _.cloneDeep(globalPlugins);
