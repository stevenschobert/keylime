...
(function (root, factory) {
   ...
   */
  KeylimeConstructor.prototype.create = function create(attrs) {
  ...
  function setAttributesUsingMapAndValues(target, map, values) {
    var mapEntry, value, handlerValue;
    ...
    for (var mapKey in map) {
      if (map.hasOwnProperty(mapKey)) {
        ...
        if (typeof mapEntry.handler === 'function') {
          handlerValue = mapEntry.handler(value, target);
          if (handlerValue !== undefined) {
            target[mapEntry.name] = handlerValue;
          }
        } else {
