...
(function (root, factory) {
  ...
  KeylimeConstructor.prototype.on = function on(event, handler) {
    ...
    if (event.toLowerCase() === 'init') {
      this.descriptor.addInitHandler(handler);
    } else if (event.toLowerCase() === 'attr' && eventName) {
      this.descriptor.addAttrHandler(eventName, handler);
    } else if ((eventSearch = attrRegex.exec(event)) !== null) {
      this.descriptor.addAttrHandler(eventSearch[1], handler);
    }
   ...
   */
  KeylimeDescriptor.prototype.removeAllInitHandlers = function removeAllInitHandlers(handler) {
