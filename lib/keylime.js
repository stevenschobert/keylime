...
(function (root, factory) {
  ...
  /**
   * "Includes" provide mixin ability to KeylimeConstructors. This works very similarly to helpers,
   * except that it does not permantly extend KeylimeConstructor.prototype.
   *
  ...
  KeylimeConstructor.prototype.on = function on(event, handler) {
    ...
    if ((eventSearch = attrRegex.exec(event)) !== null) {
      this.descriptor.addHandler(eventSearch[1], handler);
    }
   ...
   */
  KeylimeDescriptor.prototype.addHandler = function addHandler(attrName, handler) {
    var attr;
   ...
   */
  KeylimeDescriptor.prototype.removeAllHandlers = function removeAllHandlers(attrName) {
    var handlers;
   ...
   */
  KeylimeDescriptor.prototype.removeHandler = function removeHandler(attrName, handler) {
  ...
  /**
   * Registers a new helper with keylime. This provides an easy API for consumers to register
   * keylime "extensions" that automatically become part of Keylime's chainable syntax. It also
   * should not have any negative effects on keylime as a whole, since the helper functions don't
   * get invoked until the helper actually gets called.
   *
   ...
   * @param   {function}    helper  - the helper function to register
   * @return  {bool}                - if the plugin was registered
   */
  function registerHelper(name, helper) {
    assert(typeof name === 'string', 'You must supply a \'name\' parameter to register a new Keylime helper.');
    assert(typeof helper === 'function', 'You must supply a \'function\' parameter to register a new Keylime helper.');
    assert(KeylimeConstructor.prototype[name] === undefined, 'Cannot register a plugin using the name \''+name+'\', it already exists!');
    KeylimeConstructor.prototype[name] = function keylimeHelper() {
      var args = [].slice.call(arguments);
      args.unshift(this);
      helper.apply(helper, args);
      return this;
    };
    return true;
  }
  ...
  /**
   * Un-registers a helper function with Keylime.
   *
   * @param   {string}    name    - the name of the helper to unregister
   * @return  {bool}              - if the plugin was unregistered (false if not present)
   */
  function unregisterHelper(name) {
    assert(typeof name === 'string', 'You must supply a \'name\' parameter to un-register a Keylime helper.');
  ...
  /**
   * Helper for testing if objects are arrays.
   *
  ...
  keylime.registerHelper = registerHelper;
  keylime.unregisterHelper = unregisterHelper;
