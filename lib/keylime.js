(function (root, factory) {
   ...
   */
  KeylimeConstructor.prototype.attr = function attr(name, defaultValue) {
    assert(typeof name === 'string', 'A \'name\' string is required to create an attribute on '+this.name);
    this.prototype.__keylime__.attrs[name] = {
      name: name,
  ...
  function setAttributesUsingMapAndValues(target, map, values) {
    ...
    assert(typeof target === 'object' || typeof target === 'function', 'A \'target\' object is required to set attribute values.');

    for (var mapKey in map) {
      if (map.hasOwnProperty(mapKey)) {
        mapEntry = map[mapKey];
        if (values && values[mapEntry.name] !== undefined) {
          if (typeof values[mapEntry.name] === 'function') {
            target[mapEntry.name] = values[mapEntry.name]();
          } else {
            target[mapEntry.name] = values[mapEntry.name];
          }
        } else {
          if (typeof mapEntry.defaultValue === 'function') {
            target[mapEntry.name] = mapEntry.defaultValue();
          } else {
            target[mapEntry.name] = mapEntry.defaultValue;
          }
        }
